...
def test_model(test_data, model_name):
    ...
    vectorized_df = vectorize_data(vectorizer_type='tf_idf', vectorizer=vectorizer, save=False, df=cleaned_df)
    
    vectorized_df = vectorized_df.dropna()
    
    y_test = vectorized_df.iloc[:, -1] # .replace({'Yes': 1, 'No': 0}).astype(int)
...
if __name__ == '__main__':
    test_data = pd.read_csv(os.path.join(paths.test_data, 'project-4-at-2023-11-01-05-49-078c8ecf.csv'))
