...
def vectorize_and_save(df, vectorizer, vectorizer_type, labels, save_path=None, file_name=None, save=True):
    ...
    if vectorizer_type == 'tf_idf':
        ...
        raise ValueError(
            "Invalid vectorizer_type. Must be 'tf_idf' or 'incidence_matrix'.")
    ...
    if save:
        if save_path is None or file_name is None:
            raise ValueError(
                "save_path and file_name must be provided when save is True")

        file_suffix = f'_{file_name}' if file_name else ''
        file_path = os.path.join(
            save_path, f'{vectorizer_type}{file_suffix}.csv')
    ...
    if df is None:
        if filepath is None:
            filepath = os.path.join(
                paths.saved_data_cleaned, 'cleaned_merged_training_data.csv')
